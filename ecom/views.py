...
def search_view(request):
    ...
    products=models.Product.objects.all().filter(Q(name__icontains=query) | Q(description__icontains=query) | Q(category__icontains=query))
    if 'product_ids' in request.COOKIES:
        product_ids = request.COOKIES['product_ids']
        counter=product_ids.split('|')
        product_count_in_cart=len(set(counter))
...
# shipment address before placing order
@login_required(login_url='customerlogin')
def customer_address_views(request,pk):
    # this is for checking whether product is present in cart or not
    # if there is no product in cart we will not show address form
    ...
    if request.method == 'POST':
        ...
        if addressForm.is_valid():
            ...
            #         for p in products:
            #             total=total+p.price
            products=models.Product.objects.all().filter(id__in = [pk])
            for p in products:
                total=total+p.price
            response = render(request, 'ecom/payment.html',{'total':total})
            response.set_cookie('email',email)
